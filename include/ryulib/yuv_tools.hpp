...
static void RGBtoYUV420(const unsigned char* rgb, unsigned char* yuv,
	int srcFrameWidth, int srcFrameHeight, int pixelSize)
{
	...
	for (y = 0; y < srcFrameHeight; y++) {
		...
		for (x = 0; x < (int) srcFrameWidth; x += 2) {
			rgbtoyuv(rgbIndex[0], rgbIndex[1], rgbIndex[2], *yline, *uline,
				*vline);
			rgbIndex += pixelSize;
			yline++;
			rgbtoyuv(rgbIndex[0], rgbIndex[1], rgbIndex[2], *yline, *uline,
...
static bool I420ToARGB(const unsigned char* src_y, int src_stride_y,
